...
var RegisterView = React.createClass({
    ...
    render: function(){
        ...
        return (
            <View style={[styles.container]}>
                ...
                <FormBox>
                    <FormBox.Input placeholder={"网站/软件/电脑/其他名称提示"} keyboardType={"default"} maxLength={50}
                                   parent={this} paramName={"siteName"} height={30}/>
                    <FormBox.Input placeholder={"账号提示"} keyboardType={"default"}  maxLength={50}
                                   parent={this} paramName={"accountName"} height={30}/>
                    <FormBox.Input placeholder={"密码提示"} keyboardType={"default"}  maxLength={50}
                                   parent={this} paramName={"pwd"} height={30}/>
                </FormBox>
                <ButtonsBox>
                    <ButtonsBox.Button btnText={"添加"} onPress={this._onPressRegister} isPressing={this.state.isPressingRegister}/>
                </ButtonsBox>
            </View>
        );
    },
    ...
    _onPressRegister: function(){
        ...
        if(!param.siteName){
            RNUtils.alert("请输入网站/软件/电脑/其他名称提示");
            return;
        }
        if(!param.accountName){
            RNUtils.alert("请输入账号提示");
            return;
        }
        if(!param.pwd){
        ...
        param.uuid = Math.uuidFast();
        RNUtils.addJsonFromAccountPwds(global.YrcnApp.now.pwdKey,param,function(){
