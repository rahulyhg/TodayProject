...
var ListViewAccountPwd = React.createClass({
    ...
    componentDidMount: function(){
        ...
        var _this = this;
        RNUtils.getJsonFromAccountPwds(global.YrcnApp.now.pwdKey,function(data){
            console.log(data);
        ...
        RNUtils.openUpdDeleteCancel("请选择您的操作",function(btnIndex){
            if(btnIndex == 0){
                global.YrcnApp.now.rootNavigator.push({
                    name:"NavigatorAccountPwdInner",
                    title:"详情",
                    indexName:"AccountPwdDetailView",
                });
            }else if(btnIndex == 1){
                global.YrcnApp.now.rootNavigator.push({
                    name:"NavigatorAccountPwdInner",
                    title:"编辑",
                    indexName:"AccountPwdUpdView",
                });
            }else if(btnIndex == 2){
                RNUtils.delJsonFromAccountPwds(global.YrcnApp.now.pwdKey,rowData, function () {
                    _this.refreshDs();
                })
            }
        })
